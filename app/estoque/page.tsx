...
export default function Estoque() {
  ...
  const [activeTab, setActiveTab] = useState<"produtos" | "movimentacoes">(
    "produtos",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        `Erro ao salvar produto: ${error.message || "Erro desconhecido"}. Verifique se as variáveis de ambiente do Supabase estão configuradas no Vercel e se o RLS está desativado.`,
      );
    }
  };
  ...
  const getCategoryIcon = (cat: string, size = 24) => {
    ...
    if (lowerCat.includes("shampoo") || lowerCat.includes("cabelo"))
      return <Scissors size={size} />;
    if (
      lowerCat.includes("makeup") ||
      lowerCat.includes("maquiagem") ||
      lowerCat.includes("batom")
    )
  ...
  const inStockProducts = useMemo(
    () => (showOutOfStock ? products : products.filter((p) => p.quantity > 0)),
    [showOutOfStock, products],
  );
  const favorites = useMemo(
    () => inStockProducts.filter((p) => p.isFavorite),
    [inStockProducts],
  );
  ...
  const displayedProducts = useMemo(
    ...
    [selectedCategory, isViewingAll, inStockProducts],
  );
    ...
    <div className="min-h-screen bg-background-light">
      ...
      <main className="max-w-2xl mx-auto pb-24">
        <div className="px-4 pt-4">
          <h2 className="text-slate-900 text-lg font-bold leading-tight text-center uppercase tracking-widest text-primary/80 mb-4">
            Estoque de Produtos
          </h2>

          <div className="flex bg-primary/5 p-1 rounded-xl mb-4">
            <button
              onClick={() => setActiveTab("produtos")}
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${activeTab === "produtos" ? "bg-white text-primary shadow-sm" : "text-slate-500 hover:text-primary"}`}
            >
              Produtos
            </button>
            <button
              onClick={() => setActiveTab("movimentacoes")}
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${activeTab === "movimentacoes" ? "bg-white text-primary shadow-sm" : "text-slate-500 hover:text-primary"}`}
            >
              Movimentações
            </button>
          </div>
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
                {favorites.length === 0 ? (
                  ...
                  favorites.map((fav) => (
                    <div
                      key={fav.id}
                      className="min-w-[180px] bg-white p-5 rounded-2xl border border-gold/30 shadow-md flex flex-col gap-3 relative group shrink-0 hover:border-gold/60 transition-colors"
                    >
                      <button
                      ...
                      <div className="mt-1">
                        ...
                        <p className="text-slate-900 font-bold text-base leading-tight line-clamp-2 pr-4">
                          {fav.name}
                        </p>
                      </div>
                      ...
                      <div className="mt-auto pt-2 border-t border-slate-100">
                        <p className="text-primary font-black text-lg">
                          R${" "}
                          {(fav.price || 0).toLocaleString("pt-BR", {
            ...
            <div className="px-4 py-2 flex flex-col gap-3">
              ...
              {displayedProducts.length === 0 ? (
                ...
                displayedProducts.map((product) => (
                  <div
                    key={product.id}
                    className="flex items-center gap-4 bg-white p-4 rounded-xl border border-primary/5 shadow-sm"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-slate-500 text-xs truncate">
                        {product.category} • {product.quantity} un. em estoque
                      </p>
                    </div>
                      ...
                      {editingProductId === product.id ? (
                        <div className="flex items-center gap-1 mb-1">
                          <input
                            ...
                            value={editPrice}
                            onChange={(e) =>
                              setEditPrice(formatPrice(e.target.value))
                            }
              ...
              <div className="flex flex-col gap-3">
                {movements.map((movement) => {
                  ...
                  return (
                    <div
                      key={movement.id}
                      className="bg-white p-4 rounded-xl border border-primary/5 shadow-sm flex items-center gap-4"
                    >
                      <div
                        className={`size-12 rounded-full flex items-center justify-center shrink-0 ${isEntry ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600"}`}
                      >
                        {isEntry ? (
                          <ArrowUp size={20} />
                        ) : (
                      ...
                      <div className="text-right shrink-0">
                        <p
                          className={`font-bold text-lg ${isEntry ? "text-emerald-600" : "text-red-600"}`}
                        >
                          {isEntry ? "+" : "-"}
                          {movement.quantity}
                        </p>
