...
export default function Vendas() {
  ...
  const handleWhatsApp = () => {
    ...
    let message = `Olá ${selectedClient.name}! 🌸\n\n`;
    ...
    message += `\n💰 Valor Total: *${formatCurrency(currentSale.totalValue)}*\n\n`;

    if (currentSale.remainingValue > 0) {
      message += `✅ Valor Pago: *${formatCurrency(currentSale.amountPaid)}*\n`;
      message += `⏳ Restante a pagar: *${formatCurrency(currentSale.remainingValue)}* (para o próximo mês)\n\n`;
    }
              ...
              {cart.map(item => (
                <div key={item.product.id} className="p-3 border-b border-slate-100 last:border-0 flex justify-between items-center">
                  <div className="flex-1">
                    ...
                    <p className="text-primary font-bold text-xs">{formatCurrency(item.product.price)} un.</p>
                  </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-primary/10 shadow-sm space-y-3">
            <label className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-700">Pagamento Parcial (Fiado)</span>
              </div>
                ...
                {currentSale.remainingValue > 0 ? (
                  <>
                    ...
                    <div className="flex justify-between items-center text-orange-500">
                      <p className="font-semibold">Restante (Fiado)</p>
