...
export default function Vendas() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [fetchedClients, fetchedProducts, fetchedSales] =
          await Promise.all([
            api.getClients(),
            api.getProducts(),
            api.getSales(),
  ...
  const handleAddClient = async () => {
    ...
    try {
      ...
      alert(
        `Erro ao adicionar cliente: ${error.message || "Erro desconhecido"}. Verifique se as variáveis de ambiente do Supabase estão configuradas no Vercel e se o RLS está desativado.`,
      );
    }
  };
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      alert(
        `Erro ao adicionar produto: ${error.message || "Erro desconhecido"}. Verifique se as variáveis de ambiente do Supabase estão configuradas no Vercel e se o RLS está desativado.`,
      );
    }
  };
  ...
  const handleAddToCart = () => {
    ...
    if (currentQuantityInCart + 1 > product.quantity) {
      alert(
        `Estoque insuficiente. Você só tem ${product.quantity} unidades de ${product.name} em estoque.`,
      );
      return;
    }
    ...
    setCart(
      cart.map((item) => {
        if (item.product.id === productId) {
          ...
          if (newQ > item.product.quantity) {
            alert(
              `Estoque insuficiente. Você só tem ${item.product.quantity} unidades de ${item.product.name} em estoque.`,
            );
            return item;
          }
  ...
  const handleEditCartItemPrice = (productId: string) => {
    ...
    const numericPrice = parseFloat(
      editCartItemPrice.replace(/\./g, "").replace(",", "."),
    );
  ...
  const handleFinalizarVenda = async () => {
    ...
    const paid =
      paymentStatus === 'pago' ? totalValue :
      paymentStatus === 'parcial' && amountPaid ? parsePriceInput(amountPaid) :
      0;
    ...
    try {
      ...
      alert(
        `Erro ao finalizar venda: ${error.message || "Erro desconhecido"}. Verifique se as variáveis de ambiente do Supabase estão configuradas no Vercel e se o RLS está desativado.`,
      );
    }
  };
  ...
  const handleDeleteSale = async (saleId: string) => {
    ...
    try {
      ...
      // Update local products state to reflect reverted stock
      const updatedProducts = await api.getProducts();
      setProducts(updatedProducts);
    } catch (error) {
  ...
  const handleEditSale = (sale: Sale) => {
    ...
    if (sale.amountPaid === 0) {
      ...
      setAmountPaid("");
    } else if (sale.amountPaid < sale.totalValue) {
      setPaymentStatus('parcial');
      setAmountPaid(
        sale.amountPaid.toLocaleString("pt-BR", {
    ...
    // Reconstruct cart and temporarily adjust local product stock to allow editing
    let newCart: CartItem[] = [];
    const adjustedProducts = [...products];

    if (sale.items && sale.items.length > 0) {
      newCart = sale.items.map((item) => {
        const actualProduct = products.find((p) => p.id === item.productId);

        if (actualProduct) {
          // Temporarily add the sold quantity back to the local product state
          const productIndex = adjustedProducts.findIndex(
            (p) => p.id === actualProduct.id,
          );
          if (productIndex !== -1) {
            adjustedProducts[productIndex] = {
              ...adjustedProducts[productIndex],
              quantity: adjustedProducts[productIndex].quantity + item.quantity,
            };
          }
      ...
      // Legacy sale support
      const actualProduct = products.find((p) => p.id === sale.productId);
      if (actualProduct) {
        const productIndex = adjustedProducts.findIndex(
          (p) => p.id === actualProduct.id,
        );
        if (productIndex !== -1) {
          adjustedProducts[productIndex] = {
            ...adjustedProducts[productIndex],
            quantity: adjustedProducts[productIndex].quantity + 1,
          };
        }
        newCart = [{ product: actualProduct, quantity: 1 }];
      }
    }
  ...
  const handleWhatsApp = (saleToSend?: Sale) => {
    ...
    if (settings.storeName) {
      message = `Olá ${client.name}! Aqui é da *${settings.storeName}* ${emojiFlower}\n\n`;
    }
    ...
    if (sale.paymentMethod === "Pix" && settings.pixKey) {
      message += `Nossa chave PIX é: *${settings.pixKey}*\n\n`;
    }
  ...
  const handleExportExcel = () => {
    const data = filteredSales.map((sale) => {
      ...
      return {
        ...
        "Valor Total": sale.totalValue,
        "Valor Pago": sale.amountPaid,
        "Valor Restante": sale.remainingValue,
        "Método de Pagamento": sale.paymentMethod,
        "Itens": sale.items?.map(i => `${i.quantity}x ${i.name}`).join(", ") || "",
      };
    });
  ...
  const handleExportPDF = () => {
    const columns = ["Data", "Cliente", "Total", "Pago", "Restante", "Método"];
    const data = filteredSales.map((sale) => {
      const client = clients.find((c) => c.id === sale.clientId);
      return [
    ...
    exportToPDF("Relatório de Vendas", columns, data, "vendas");
  };
        ...
        {editingSaleId && activeTab === "nova" && (
          <div className="flex justify-between items-center bg-amber-50 border border-amber-200 p-3 rounded-xl">
            ...
            <button
              ...
              className="text-amber-700 hover:text-amber-900 text-sm font-bold flex items-center gap-1"
            >
              <X size={16} /> Cancelar
            </button>
          </div>
              ...
              <div className="flex justify-between items-center">
                ...
                {!isAddingClient && (
                  <button
                    ...
                    className="text-primary flex items-center gap-1 text-xs font-bold bg-primary/10 px-2 py-1 rounded-lg"
                  >
                ...
                <div className="bg-white p-4 rounded-xl border border-primary/20 space-y-3 shadow-sm">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 h-10 bg-slate-100 text-slate-600 rounded-lg font-bold"
                    >
                      Cancelar
                    </button>
                    <button
                      onClick={handleAddClient}
                      className="flex-1 h-10 bg-primary text-white rounded-lg font-bold"
                    >
                      Salvar
                    </button>
                  </div>
                ...
                <div className="relative">
                  <label className="flex flex-col gap-1.5">
                    <div className="relative">
                      ...
                      <User
                        className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-gold"
                        size={20}
                      />
                      <ChevronDown
              ...
              {isAddingProduct ? (
                <div className="bg-white p-4 rounded-xl border border-primary/20 space-y-3 shadow-sm">
                  <p className="text-xs text-slate-500 mb-2">
                    Adicione um produto do catálogo ou fora do estoque.
                  </p>
                  <input
                    type="text"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewProductPrice(formatPriceInput(e.target.value))
                    }
                    className="w-full rounded-lg border-primary/20 bg-background-light focus:border-primary focus:ring-1 focus:ring-primary h-12 px-4 outline-none"
                  />
                  <div className="flex gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        {editingCartItemId === item.product.id ? (
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            <button
                              onClick={() =>
                                handleEditCartItemPrice(item.product.id)
                              }
                          ...
                          <div
                            ...
                            onClick={() => {
                              ...
                              setEditCartItemPrice(
                                formatPriceInput(item.product.price.toFixed(2)),
                              );
                            }}
                          >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center bg-slate-100 rounded-lg">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center text-slate-600 font-bold"
                          >
                            -
                          </button>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <label className="cursor-pointer group">
                  <input
                    type="radio"
                    name="payment"
                    value="Link de Cartão"
                    checked={paymentMethod === "Link de Cartão"}
                    onChange={(e) => setPaymentMethod(e.target.value)}
                    className="peer hidden"
                  />
                  <div className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-primary/10 bg-white peer-checked:border-gold peer-checked:bg-gradient-to-br peer-checked:from-primary/5 peer-checked:to-gold/10 transition-all group-hover:bg-primary/5">
                ...
                <label className="cursor-pointer group">
                  <input
                    ...
                    name="payment"
                    value="Parcelado 2x"
                    checked={paymentMethod === "Parcelado 2x"}
                    onChange={(e) => setPaymentMethod(e.target.value)}
                    className="peer hidden"
                  />
                  <div className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-primary/10 bg-white peer-checked:border-gold peer-checked:bg-gradient-to-br peer-checked:from-primary/5 peer-checked:to-gold/10 transition-all group-hover:bg-primary/5">
                    <span
                      className={`text-lg font-black mb-1 ${paymentMethod === "Parcelado 2x" ? "text-gold" : "text-primary"}`}
                    >
                      2x
                    </span>
                ...
                <label className="cursor-pointer group">
                  <input
                    ...
                    name="payment"
                    value="Parcelado 3x"
                    checked={paymentMethod === "Parcelado 3x"}
                    onChange={(e) => setPaymentMethod(e.target.value)}
                    className="peer hidden"
                  />
                  <div className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-primary/10 bg-white peer-checked:border-gold peer-checked:bg-gradient-to-br peer-checked:from-primary/5 peer-checked:to-gold/10 transition-all group-hover:bg-primary/5">
                    <span
                      className={`text-lg font-black mb-1 ${paymentMethod === "Parcelado 3x" ? "text-gold" : "text-primary"}`}
                    >
                      3x
                    </span>
                    <span className="text-sm font-semibold text-center">
                ...
                <label className="cursor-pointer group">
                  <input
                    ...
                    name="payment"
                    value="Parcelado 4x"
                    checked={paymentMethod === "Parcelado 4x"}
                    onChange={(e) => setPaymentMethod(e.target.value)}
                    className="peer hidden"
                  />
                  <div className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-primary/10 bg-white peer-checked:border-gold peer-checked:bg-gradient-to-br peer-checked:from-primary/5 peer-checked:to-gold/10 transition-all group-hover:bg-primary/5">
                    <span
                      className={`text-lg font-black mb-1 ${paymentMethod === "Parcelado 4x" ? "text-gold" : "text-primary"}`}
                    >
                      4x
                    </span>
                    <span className="text-sm font-semibold text-center">
              ...
              <div className="bg-white p-4 rounded-xl border border-primary/10 shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className={`py-2 text-sm font-bold rounded-lg border transition-colors ${paymentStatus === 'pago' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                  >
                    Pago
                  </button>
                  <button
                    ...
                    className={`py-2 text-sm font-bold rounded-lg border transition-colors ${paymentStatus === 'parcial' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                  >
                    Parcial
                  </button>
                  <button
                    ...
                    className={`py-2 text-sm font-bold rounded-lg border transition-colors ${paymentStatus === 'a_receber' ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                  >
                    A Receber
                  </button>
                </div>
                      ...
                      <div className="w-full rounded-lg border border-slate-200 bg-slate-50 h-10 px-3 flex items-center text-sm font-bold text-orange-500">
                        {formatCurrency(
                          Math.max(
                            0,
                            totalValue -
                              (amountPaid ? parsePriceInput(amountPaid) : 0),
                          ),
                        )}
                      </div>
                    </div>
                ...
                {paymentStatus === 'a_receber' && (
                  <div className="pt-3 border-t border-slate-100">
                    <p className="text-xs text-orange-600 font-medium bg-orange-50 p-2 rounded border border-orange-100">
                      O valor total de {formatCurrency(totalValue)} será registrado como &quot;Fiado&quot; (A Receber).
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 rounded-xl border border-primary/10 bg-white flex items-start gap-3">
                ...
                <p className="text-xs leading-relaxed text-slate-600">
                  As opções de parcelamento no cartão estarão disponíveis no
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded hover:bg-red-100 transition-colors"
                  >
                        ...
                        {sale.remainingValue > 0 && (
                          <div className="flex justify-between items-center text-xs pt-3 mt-2 border-t border-slate-100">
                            ...
                            <span className="text-orange-500 font-bold">
                              Fiado: {formatCurrency(sale.remainingValue)}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-600 flex items-center gap-1">
                  <MapPin size={14} />{" "}
                  {selectedClient.address || "Endereço não cadastrado"}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                {currentSale.remainingValue > 0 ? (
                  <>
                    <div className="flex justify-between items-center text-emerald-600">
                      ...
                      <p className="font-bold">
                        {formatCurrency(currentSale.amountPaid)}
                      </p>
                    </div>
                    <div className="flex justify-between items-center text-orange-500">
                      <p className="font-semibold">Restante</p>
                      <p className="font-bold">
                        {formatCurrency(currentSale.remainingValue)}
                      </p>
                    </div>
                  </>
                ) : (
                  <div className="flex justify-between items-center text-emerald-600">
                    <p className="font-semibold">Valor Pago</p>
                    <p className="font-bold">
                      {formatCurrency(currentSale.totalValue)}
                    </p>
                  </div>
                )}
              </div>
